...
class CalTime(object):
    ...
    def _check_precise(self, pic_index, pic_list, pic_dir, target_stage):
        ...
        pic_path = os.path.join(pic_dir, pic_list[pic_index])
        id_ret = self.classifier.identify_pic(pic_path)
        # end 通过 「words」的 upperbound 往后找，loading 通过「logo」的upperbound 向后找
        direction = 1
        length = len(pic_list)
        msg = {}
        ...
        return pic_index, id_ret if id_ret[0] == target_stage else pic_index - 1, last

    def _check_ad(self, pic_index, pic_list, pic_dir):
        '''
        检查从 pic_index 开始，有没有出现 ad，但是只往后检查 10 张图片
        :param pic_index:
        :param pic_list:
        :param pic_dir:
        :return:
    ...
    def cal_time(self, pic_dir, exclude_list, start_time):
        ...
        for stage in self.SORTED_STAGE:
            ...
            search_method = self.upper_bound if stage == 'words' or stage == 'logo' else self.lower_bound
            is_upper_bound = True if stage == 'words' or stage == 'logo' else False
            self.progress = 0
            bound_index = search_method(pic_dir, pic_list, 0, length, ZHIHU_SORTED_STAGE[stage], stage)
            if bound_index == -1 or self._check_ad(bound_index, pic_list, pic_dir):
                self.ad_exist = True
        ...
        if self.ad_exist:
            ...
            str2 = "文件夹 %s: App 启动时长：%.3fs" % (self.times_counter, launch_time)
            info = str2
            print(str2)
        else:
            # str3 = "高端机型不存在「loading」阶段，因此首页加载时长为 0"
            str4 = "文件夹 %s: App 启动时长: %.3fs" % (self.times_counter, launch_time)
            info = str4
            # print(str3)
            print(str4)
        ...
        self.shared_answer_queue.put(json.dumps(msg))
    ...
    def _find_first_end(self, start_index, pic_dir, pic_list, index_ls):
        ...
        while 0 <= start_index < length:
            ...
            pic_path = os.path.join(pic_dir, pic_name)
            id_ret = self.classifier.identify_pic(pic_path)
            if id_ret[0] == 'ad' and id_ret[1] >= 0.9:
                return -2, None
...
if __name__ == '__main__':
    ...
    print (CalTime.get_create_time("2019-11-29_21-13-53-491968.jpg"))
    # msg = {
    #     'dirname': '1',
    #     'summary': '共有 x 张图片',
